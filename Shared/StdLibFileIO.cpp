...
int CStdLibFileIO::Open(const wchar_t * pName, BOOL bOpenReadOnly)
{
    ...
    {
	ALOGI("%s %d %s",__func__, __LINE__, CAPECharacterHelper::GetANSIFromUTF16(pName));
        //CSmartPtr<char> spFilenameANSI((char *) CAPECharacterHelper::GetANSIFromUTF16(pName), TRUE);
		char* spFilenameANSI = CAPECharacterHelper::GetANSIFromUTF16(pName); 
        m_pFile = fopen(spFilenameANSI, "rb");
